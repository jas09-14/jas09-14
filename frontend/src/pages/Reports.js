        ...
        <div className="bg-card rounded-2xl border border-border/50 p-6 shadow-soft">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-border/50">
              ...
              <span className="font-mono font-semibold text-foreground">
                R$ {((summary?.total_planned || 0) / 12).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex justify-between items-center py-2 border-b border-border/50">
              <span className="text-muted-foreground">Média Realizada</span>
              <span className="font-mono font-semibold text-foreground">
                R$ {((summary?.total_actual || 0) / 12).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex justify-between items-center py-2">
              <span className="text-muted-foreground">Variação Média</span>
              <span className="font-mono font-semibold text-muted-foreground">
                R$ {(Math.abs((summary?.total_planned || 0) - (summary?.total_actual || 0)) / 12).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </span>
            </div>
          </div>
        </div>
